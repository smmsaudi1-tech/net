...
export function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    ...
    SEGMENTS.push(dive);
    s._seg = dive;
    if (i < N - 1 && (CONNECTORS[i] || CONNECTORS_M[i])) {
  ...
  function jumpTo(i) {
    ...
    const containerTop = container.getBoundingClientRect().top + (window.scrollY || window.pageYOffset);
    window.scrollTo({ top: containerTop + seg.start + (seg.end - seg.start) * 0.5, behavior: reduce ? 'auto' : 'smooth' });
  }

  function loadClip(s) {
    if (reduce || s.loading || (!s.clip && !s.clipM)) return;
    s.loading = true;
    const url = (isMobile() && s.clipM) ? s.clipM : (s.clip || s.clipM);
    if (!url) return;
  ...
  function read() {
    ...
    const y = clamp(scrollOffset, 0, totalW * vh);
...
function injectCSS() {
  ...
  const css = `
  .sw-root{--sw-bg:#000000;--sw-ink:#ffffff;--sw-ink-soft:#a1a1aa;--sw-accent:#8b5cf6;
    ...
    color:var(--sw-ink);font-family:var(--sw-font-body);position:relative;width:100%;}
  ...
  .sw-nav{display:flex;gap:4px;padding:5px;background:color-mix(in srgb,#000 65%,transparent);backdrop-filter:blur(10px);border:1px solid color-mix(in srgb,var(--sw-accent) 26%,transparent);border-radius:999px;pointer-events:auto;}
  ...
  .sw-topcta{text-decoration:none;font-weight:600;font-size:.9rem;color:#000;background:#fff;padding:10px 20px;border-radius:999px;white-space:nowrap;pointer-events:auto;}
  ...
  .sw-copy__tags li{font-size:.82rem;font-weight:600;color:color-mix(in srgb,var(--sw-accent) 90%,#fff);padding:7px 14px;border-radius:999px;background:color-mix(in srgb,var(--sw-accent) 20%,#000);border:1px solid color-mix(in srgb,var(--sw-accent) 40%,transparent);}
  ...
  .sw-btn--primary{color:#000;background:#fff;} .sw-btn--primary:hover{transform:translateY(-2px);}
  .sw-btn--ghost{color:#fff;border:1.5px solid color-mix(in srgb,#fff 35%,transparent);} .sw-btn--ghost:hover{transform:translateY(-2px);}
  ...
  .sw-route::before{content:"";position:absolute;left:50%;top:22px;bottom:22px;width:2px;transform:translateX(-50%);background:var(--sw-accent);opacity:.38;}
  ...
  .sw-route__dot i{width:9px;height:9px;border-radius:50%;background:color-mix(in srgb,var(--sw-accent) 50%,transparent);transition:transform .3s,background .3s,box-shadow .3s;}
  ...
  .sw-route__label{position:absolute;right:24px;top:50%;transform:translateY(-50%) translateX(6px);white-space:nowrap;font-size:.78rem;font-weight:600;color:var(--sw-ink);background:color-mix(in srgb,#000 85%,transparent);backdrop-filter:blur(6px);padding:5px 11px;border-radius:999px;opacity:0;pointer-events:none;transition:opacity .25s,transform .25s;border:1px solid color-mix(in srgb,var(--sw-accent) 24%,transparent);}
